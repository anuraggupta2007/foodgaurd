...
import type { AppLanguage } from "@/data/languages";
...
export function LanguageOption({
  ...
  const displayName = `${language.flag} ${language.nativeLabel}`;
  ...
  return (
    <button
      ...
      className={cn(
        "group flex w-full items-center justify-between gap-3 rounded-xl border px-4 py-3.5 text-left transition-[background-color,border-color,box-shadow] duration-150",
