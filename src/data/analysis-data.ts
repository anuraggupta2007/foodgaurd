...
export function getAnalysisFromIngredients(
  ...
  for (const [key, data] of Object.entries(INGREDIENT_MAP)) {
    if (seen.has(key)) continue;
    if (lower.includes(key)) {
      found.push({ name: key.charAt(0).toUpperCase() + key.slice(1), ...data });
      seen.add(key);
    }
  }
  ...
  const score = Math.max(10, 100 - highCount * 20 - moderateCount * 8);
